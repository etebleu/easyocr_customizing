...
class Model(nn.Module):
    def __init__(self, input_channel, output_channel, hidden_size, num_class):
        ...
        recog_network = os.path.splitext(os.path.basename(__file__))[0]
        with open(os.path.join(base_path, recog_network+ '.yaml'), encoding='utf8') as file:
            recog_config = yaml.load(file, Loader=yaml.FullLoader)
        self.config = AttrDict(recog_config)
        self.stages = {'Trans': self.config.Transformation, 'Feat': self.config.FeatureExtraction,
                       'Seq': self.config.SequenceModeling, 'Pred': self.config.Prediction}
        ...
        if self.config.Transformation == 'TPS':
            self.Transformation = TPS_SpatialTransformerNetwork(
                ...
                I_size=(self.config.imgH, self.config.imgW), 
                I_r_size=(self.config.imgH,self.config.imgW), 
        ...
        """ FeatureExtraction """
        if self.config.FeatureExtraction == 'VGG':
            self.FeatureExtraction = VGG_FeatureExtractor(input_channel, output_channel)
        elif self.config.FeatureExtraction == 'RCNN':
            self.FeatureExtraction = RCNN_FeatureExtractor(input_channel, output_channel)
        elif self.config.FeatureExtraction == 'ResNet':
            self.FeatureExtraction = ResNet_FeatureExtractor(input_channel, output_channel)
        else:
        ...
        """ Sequence modeling"""
        if self.config.SequenceModeling == 'BiLSTM':
