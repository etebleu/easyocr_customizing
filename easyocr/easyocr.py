...
class Reader(object):
    ...
    def __init__(self, lang_list, gpu=True, model_storage_directory=None,
        ...
        self.support_detection_network = ['craft', 'dbnet18', 'craft_doc_v3', 'craft_train_v2']
        ...
        if recog_network in ['standard'] + [model for model in recognition_models['gen1']] + [model for model in recognition_models['gen2']]:
            ...
            model_path = os.path.join(self.model_storage_directory, 'recognition', model['filename'])
            ...
            if recognizer:
                if os.path.isfile(model_path) == False:
                    ...
                    download_and_unzip(model['url'], model['filename'], os.path.join(self.model_storage_directory, 'recognition'), verbose)
            ...
            model_path = os.path.join(self.model_storage_directory, 'recognition', model_file)
            self.setLanguageList(lang_list, recog_config)
            
            self.batch_max_length = recog_config['batch_max_length']
            self.pad = recog_config['PAD']
    ...
    def getDetectorPath(self, detect_network):
        if detect_network in self.support_detection_network:
            ...
            elif self.detect_network in ['craft_doc_v3', 'craft_train_v2']:
                from .detection_custom import get_detector, get_textbox
            else:
            ...
            detector_path = os.path.join(self.model_storage_directory, 'detection', self.detection_models[self.detect_network]['filename'])
            if os.path.isfile(detector_path) == False:
                ...
                LOGGER.warning('Downloading detection model, please wait. '
                               'This may take several minutes depending upon your network connection.')
                download_and_unzip(self.detection_models[self.detect_network]['url'], self.detection_models[self.detect_network]['filename'], os.path.join(self.model_storage_directory, 'detection'), self.verbose)
    ...
    def detect(self, img, min_size = 20, text_threshold = 0.7, low_text = 0.4,\
        ...
        return text_box_list, horizontal_list_agg, free_list_agg
    ...
    def recognize(self, img_cv_grey, horizontal_list=None, free_list=None,\
        ...
        if ((batch_size == 1) or (self.device == 'cpu')) and not rotation_info:
            ...
            for i, bbox in enumerate(horizontal_list):
                ...
                if self.recog_network in ['generation1', 'generation2']:
                    self.batch_max_length = int(int(max_width)/10)
                    imgW = int(max_width)
                else: # cutom model
                    imgW = 100
                result0 = get_text(i, self.character, imgH, imgW, self.recognizer, self.converter, image_list,\
                            self.batch_max_length, self.pad,\
        ...
        # default mode will try to process multiple boxes at the same time
            ...
            if self.recog_network in ['generation1', 'generation2']:
                ...
                imgW = 100
            result = get_text(i, self.character, imgH, imgW, self.recognizer, self.converter, image_list,\
                        self.batch_max_length, self.pad,\
                        ignore_char, decoder, beamWidth, batch_size, contrast_ths, adjust_contrast, filter_ths,\
                        workers, self.device)
    ...
    def readtext(self, image, decoder = 'greedy', beamWidth= 5, batch_size = 1,\
        ...
        _, horizontal_list, free_list = self.detect(img, 
    ...
    def readtext_batched(self, image, n_width=None, n_height=None,\
        ...
        _, horizontal_list_agg, free_list_agg = self.detect(img, 
                                                 min_size = min_size, text_threshold = text_threshold,\
                                                 low_text = low_text, link_threshold = link_threshold,\
                                                 canvas_size = canvas_size, mag_ratio = mag_ratio,\
                                                 slope_ths = slope_ths, ycenter_ths = ycenter_ths,\
