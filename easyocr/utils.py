...
def get_image_list(horizontal_list, free_list, img, model_height = 64, sort_output = False):
    ...
    for box in horizontal_list:
        ...
        if new_width == 0:
            ...
            # save_dir = 'crop_img/(get_image_list)comute_ratio_and_resize'
            # if not os.path.exists(save_dir):
            #     os.makedirs(save_dir)
            # save_path = os.path.join(save_dir, '{}_{}_{}_{}.jpg'.format(width, height, crop_img.shape[1], crop_img.shape[0]))    
...
class AttnLabelConverter(object):
    ...
    def __init__(self, character):
        ...
        print('len self.character: {}'.format(len(self.character)))
        self.dict = {}
